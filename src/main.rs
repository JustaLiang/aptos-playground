...
async fn main() -> Result<()> {
    ...
    let node_url = std::env::var("APTOS_NODE_URL")
                            .expect("APTOS_NODE_URL not set");
    let account_address = std::env::var("ACCOUNT_ADDRESS")
                            .expect("ACCOUNT_ADDRESS not set");
    ...
    let key = AccountKey::from_private_key(
        Ed25519PrivateKey::try_from(&private_key as &[u8])?
    );
    ...
    let to_address = std::env::var("ACCOUNT_ADDRESS").unwrap_or(format!("0x{}", address.to_string()));
    let to_address = AccountAddress::from_hex_literal(&to_address)?;
    let balance = get_island_coin_balance(&client, to_address, &coin_store).await;
    ...
    // send transaction
    let chain_id = client.get_index().await.expect("Failed to fetch chain ID").inner().chain_id;
...
async fn get_island_coin_balance(client: &Client, address: AccountAddress, coin_store: &str) -> Result<u64> {
    ...
    resp.and_then(|resource| {
        if let Some(res) = resource {
            ...
            Err(anyhow!("No resource unde account").into())
        }
    }).and_then(|balance| {
       Ok(balance.inner().get())
    })
}